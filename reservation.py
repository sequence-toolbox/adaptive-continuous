...
from action_condition_set import (
    ...
    es_rule_action_A_adaptive,
    es_rule_action_B_adaptive,
)
    ...
    def create_rules_request(self, path: list, reservation: ReservationAdaptive) -> List["Rule"]:
        ...
        if index < len(path) - 1:      # non responder
            ...
            action_args = {"mid": self.owner.map_to_middle_node[path[index + 1]],
                           ...
                           "encoding_type": "single_heralded", "raw_epr_errors": [1/3, 1/3, 1/3]}
            rule = Rule(priority, eg_rule_action_request_adaptive, eg_rule_condition, action_args, condition_args)
            rules.append(rule)
            priority += 1
        ...
        if index == 0:                 # initiator
            ...
            action_args = {"encoding_type": "single_heralded"}
            rule = Rule(priority, es_rule_action_B_adaptive, es_rule_condition_B_end, action_args, condition_args)
            rules.append(rule)
            priority += 1
            ...
            rule = Rule(priority, es_rule_action_A_adaptive, es_rule_condition_A, action_args, condition_args)
            rules.append(rule)
            priority += 1

            action_args = {"encoding_type": "single_heralded"}
            rule = Rule(priority, es_rule_action_B_adaptive, es_rule_condition_B, action_args, condition_args)
            rules.append(rule)
            priority += 1
